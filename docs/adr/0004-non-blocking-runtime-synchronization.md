...
reconciliation. Work is split into at-most-seven-day supply-point windows,
serialized through one shared request gate, checkpointed only after the affected
ledger data is durably flushed, and reconstructed after restart.
...
direction from successful authoritative batches. Global schema capability does
not by itself create an entity.
...
- Month and week entities may remain unknown until authoritative query coverage
  reaches their complete calendar window.
- A private versioned checkpoint store is required for each initialized supply
  point.
