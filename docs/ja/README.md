...
|---|---|---|---|
| 現在利用できるか | **はい** | クライアント ID が必要 | クライアント ID が必要 |
...
**デバイスコード** と **アカウント** は OAuth 方式です。いずれも実装済みですが、
**設定 → デバイスとサービス → アプリケーション認証情報** に登録する公開 OAuth
クライアント ID が必要です。この事業者向けのクライアント ID は公開されていないため現在は
完了できず、選択すると認証情報の追加を促すメッセージで停止します。デバイスコードは
ブラウザーのリダイレクトを必要としないため、外部から到達できない Home Assistant でも
利用できます。
...
| OAuth クライアント ID | **アプリケーション認証情報** | OAuth の 2 方式 | 公開クライアント ID。秘密情報ではないため、シークレット欄は空のままにします |
...
売電側の料金統計はありません。逆潮流の電力は使用量とは別の仕組みで精算されるため、
使用量の単価で計算すると存在しない支払いを作り出すことになります。**系統への逆潮流**
の料金欄は空のままにしてください。

料金は目安として扱ってください。締め済みの請求書 1 件と比較したところ請求額の 104%
でした。理由は[制限事項](#制限事項)に記載しています。

## 制限事項

**料金は概算です。** 単価はご自身の契約から取得しているため推測ではありません。差が
生じる要因は 2 つあります。請求は深夜すぎの検針時刻で区切られる一方、ここでの段階料金は
Asia/Tokyo の暦月で区切り直されます。また燃料費調整額は当月分のみ取得できるため、
それより前の月の時間帯には適用されません。

**事業者が返す区間ごとの料金値は表示しません。** 燃料費調整額と賦課金を 1 つの数値に
まとめており、基本料金を表現できないため、請求額を再現できません。

**初回導入時は今月と先月から始まります。** それより前の履歴も取得可能で、背景処理で
順次取得します。

**カレンダー集計は請求期間と一致しません。** 請求期間は深夜すぎの検針時刻まで、こちらは
Asia/Tokyo の暦月です。設計上、この 2 つは一致しません。
...
同期チェックポイント、インストール固有の秘密鍵、保存済みの認証情報が削除されます。
OAuth のエントリは認可の取り消しも行います。パスワード方式のエントリは、利用者側から
リフレッシュトークンを無効化できないため取り消しは行えず、7 日以内に事業者側で失効し
ます。直ちに遮断したい場合はパスワードを変更してください。

意図的に残るものが 2 つあります。

- **Energy Dashboard の統計** はレコーダーに残ります。統合を削除してエネルギー履歴が
  失われないようにするためです。**開発者ツール → 統計** から削除できます。
- **アプリケーション認証情報** は残ります。クライアント ID を再入力せずに再追加できる
  ようにするためです。**設定 → デバイスとサービス → アプリケーション認証情報** から
  削除できます。

再追加すると、ローカルの履歴は新規に始まり、今月と先月を再取得します。
