...
|---|---|---|---|
| 現在提供されるか | **はい** | いいえ — クライアント未発行 | いいえ — クライアント未発行 |
...
**デバイスコード** と **アカウント** は OAuth 方式です。実装は完了しており事業者の認可
サーバーに対して検証済みですが、いずれも事業者が発行するクライアントを必要とします。
2026 年 8 月 6 日に直接お伺いしたところ、日本国内では OAuth に対応しておらず、個人の
お客様向けの API 提供も行っていないとのご回答でした。そのため**この 2 方式は選択肢に
表示されません**。

削除はしていません。クライアント ID はコード内の空の定数であり、値が入れば両方式とも
そのまま動作します。ご自身のクライアントをお持ちの場合は、**設定 → デバイスとサービス →
アプリケーション認証情報** から追加すると選択肢に現れます。
