...
- 段階料金は、供給を開始した日を起点とする請求対象期間で区切り直します。事業者は検針の
  時刻を公開していないため、各境界は数時間ずれる可能性があります。
...
暦月です。請求は供給開始日を起点とする期間を対象とします。料金の計算はその期間に合わせて
いますが、カレンダー集計は設計上一致しません。
...
意図的に残るものが 1 つあります。**アプリケーション認証情報** は、クライアント ID を
再入力せずに再追加できるようにするため残ります。**設定 → デバイスとサービス →
アプリケーション認証情報** から削除できます。
