...
料金は概算として扱ってください。締め済みの請求書 1 件と比較したところ請求額の 104%
でした。理由は[制限事項](#制限事項)に記載しています。

## 制限事項

**料金は概算です。** 単価はご自身の契約から取得しているため推測ではありません。それでも
請求額と差が生じる要因が 2 つあります。

- 請求は 2 回の検針の間を対象とします。ここでの段階料金は Asia/Tokyo の暦月で区切り直す
  ため、対象期間が一致しません。
- 燃料費調整額は当月分のみ取得できるため、それより前の月の時間帯には適用されません。
...
**カレンダー集計は請求期間と一致しません。** ここでの集計はすべて Asia/Tokyo の暦日・暦週・
暦月です。請求は 2 回の検針の間を対象とします。設計上、この 2 つは一致しません。
