...
4. Octopus Energy Japan のメールアドレスとパスワードでサインインします。

設定はこれだけです。API キー、アカウント番号、供給地点特定番号を調べる必要はありません。
残りはサインインしたアカウントから自動的に検出されます。

### サインインについて

メールアドレスとパスワードは **Home Assistant に保存** されます。利便性のためではありま
せん。事業者のセッションは 7 日間で切れ、更新しても期限は延びないため、1 週間後には
パスワードそのもの以外に再サインインする手段がないからです。保存内容は
[`PRIVACY.md`](../../PRIVACY.md) に記載しています。

これは事業者の旧来のログイン方式であり、予告なく使えなくなる可能性があります。その場合は
Home Assistant が再サインインを求めます。

<details>
<summary>ブラウザーでサインインする方式がない理由</summary>

**アカウント（ブラウザー）** と **デバイスコード** の 2 つの OAuth 方式も実装済みで、
事業者の認可サーバーに対して検証済みです。いずれも選択肢に表示されません。どちらも事業者
が発行するクライアントを必要とし、2026 年 8 月 6 日に直接お伺いしたところ、日本国内では
OAuth に対応しておらず、個人のお客様向けの API 提供も行っていないとのご回答でした。

コードは削除せず残しています。事業者がクライアントを発行した場合、またはご自身の
クライアントを **設定 → デバイスとサービス → アプリケーション認証情報** から追加した
場合は、他に何も変更せずに両方式が選択肢に戻ります。

</details>

### あとで切り替えても履歴は残ります

統合のメニューから再接続を選んでください。使用量データと Energy Dashboard 統計は保持され、
保存済みのパスワードは削除されます。削除して再追加する必要はありません。

### 入力する項目

| 項目 | 場所 | 内容 |
|---|---|---|
| メールアドレスとパスワード | 設定時 | Octopus Energy Japan のサインイン情報 |
| この接続の名前 | 統合の **設定** | デバイス名と統計名に表示されます |
| 終了したアカウント・供給地点 | 統合の **設定** | 終了したもののうち、報告を続けるもの |

### ログインが 2 つある場合

デバイスの番号は 1 つの接続の中で振られるため、ログインが 2 つあると両方が
`OEJP account 1`、`OEJP supply point 1-1` になります。**設定** から接続ごとに名前を付けると
（たとえば「home」と「old flat」）、`OEJP home account 1` のようにデバイス名と統計名が
区別されます。

デバイスを手動で改名しても構いません。Energy Dashboard の統計名は、付けた名前に追従します。
...
プランを前提にすることもありません。それでも請求額と差が生じる要因が 4 つあります。
...
- 契約がすでに終了している場合（プラン変更、またはアカウントを解約していない退去）は、
  その契約から料金を算出しますが、現行の契約より概算の度合いが大きくなります。使用量分と
  基本料金は正確です。一方、燃料費調整額と再生可能エネルギー発電促進賦課金は、この統合
  自身が保存してきたアーカイブからしか読み取れないため、稼働中に実際に観測できなかった
  時間帯は、アーカイブにある中で最も近い値で算出します。該当した時間数は診断情報に記録
  されます。ただし契約終了までにアーカイブが片方の加算額を一度も記録していなかった場合は、
  参照できる値がないため、その加算額は 0 円として算出され、この件数には含まれません。

**時間帯別のプランにも対応しています。** EV オクトパス、オール電化オクトパス（サンシャインを
含む）、ソーラーオクトパス、動力オクトパスを、全 9 エリアで計算できます。単価はご自身の契約から
取得したものをそのまま使います。組み込んであるのは各時間帯の**時刻だけ**です。事業者は時刻を
料金メニュー定義書で公開している一方、API では該当フィールドがすべて認可エラーになるためです。
どの時刻をどの文書から読み取ったかは [docs/TOU_SCHEMES.md](../TOU_SCHEMES.md) に記載しています。
一覧にない時間帯別プランが登場した場合は、時刻を推測せずその旨をお知らせします。

**プランによっては金額を算出できません。** 消費電力量以外を単位とする料金が含まれるプランや、
上記一覧にない時間帯別プランは、この計算では表現できません。その場合は料金統計を
